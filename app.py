...
# 从 Streamlit 云端的安全 Secrets 中读取 API Key
try:
    API_KEY = st.secrets["GEMINI_API_KEY"]
except Exception:
    st.error("⚠️ 未检测到云端配置的 GEMINI_API_KEY，请前往 Streamlit 后台的 App settings -> Secrets 中配置。")
    API_KEY = ""
...
if uploaded_files:
    ...
    if st.button("🚀 开始让 AI 批量分析图片", type="primary"):
        if not API_KEY:
            st.error("请先在 Streamlit 后台配置好 API Key 才能进行分析。")
        else:
